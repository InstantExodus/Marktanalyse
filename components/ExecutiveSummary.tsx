                ...
                <ul className="space-y-3">
                    {(summaryPoints as string[]).map((point, index) => (
                        <li key={index} className="flex items-start">
                            ...
                            <p className="text-text-secondary text-[15px] leading-relaxed">{point}</p>
                        </li>
                    ))}
                </ul>
            </div>
        ...
        <div className="bg-surface p-6 rounded-2xl shadow-subtle mb-8 border border-border-color">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {structuredPoints.map((point, index) => (
                    <div key={index} className="bg-brand-orange/95 p-4 rounded-xl flex flex-col transition-all duration-300 hover:bg-brand-orange">
                        <div className="flex items-center mb-2">
                            {Icons[point.icon] || Icons['graph']}
                            <h3 className="text-lg font-semibold text-white ml-3">{point.title}</h3>
                        </div>
